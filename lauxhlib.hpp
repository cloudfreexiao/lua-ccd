...
lua_State *lauxhref_init(lua_State *L)
{
...
void lauxhref_close(lua_State *refL)
{
    lua_State *L = refL;
    lua_pushnil(L);
    lua_rawsetp(L, LUA_REGISTRYINDEX, refL);
...
bool lauxhref_isvalid(lua_State *refL, int ref)
{
...
int lauxhref_ref(lua_State *refL, lua_State *L)
{
...
void lauxhref_unref(lua_State *refL, int ref)
{
    if (ref <= FREELIST) {
        return;
    }
...
void lauxhref_get(lua_State *refL, lua_State *L, int ref)
{
    assert(lauxhref_isvalid(refL, ref));
